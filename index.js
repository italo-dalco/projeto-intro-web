...
console.log(
  `As características do ${objeto1["nome"]} são: ${objeto1["caracteristica"]}`
);
...
objeto2 = {
  ...
  timesJogados: ["Barcelona", "Paris Saint-Germain"],
};
...
console.log(
  `As características do ${objeto2["nome"]} são: ${objeto2["caracteristica"]}`
);
...
console.log(
  `As características do ${objeto3["nome"]} são: ${objeto3["caracteristica"]}`
);
...
console.log(
  `As características do ${objeto4["nome"]} são: ${objeto4["caracteristica"]}`
);

//Médias dos Jogadores -------------------------------------------

const idadeMediaJogadores =
  (objeto1["idade"] + objeto2["idade"] + objeto3["idade"] + objeto4["idade"]) /
  4;
console.log(
  `A média de idade entre os jogadores é de: ${idadeMediaJogadores} anos.`
);

const nivelMedioJogadores =
  (objeto1["nivel"] + objeto2["nivel"] + objeto3["nivel"] + objeto4["nivel"]) /
  4;
console.log(
  `A média do nível entre os jogadores é de: ${nivelMedioJogadores} estrelas.`
);
...
const formatarCaracteristica = (objeto) => {
  ...
  for (let i = 0; i < objeto.caracteristica.length; i++) {
    if (i == 2) {
      caracteristica = caracteristica + objeto.caracteristica[i];
    } else {
      caracteristica = caracteristica + objeto.caracteristica[i] + ", ";
    }
  }
...
formatarCaracteristica(objeto1);
formatarCaracteristica(objeto2);

console.log(objeto1, objeto2, objeto3, objeto4);

// Ainda no relatório, altere-o para que ele seja criado utilizando um laço. Ou seja, você não deve mais
// imprimir individualmente cada item do relatório. Cada item deve ser exibido a partir de uma iteração
// do laço. Para testar, adicione mais um item ao array de objetos, e verifique se ele é exibido corretamente.

//Não entendido (enunciado confuso)
...
const funcaoSemana3 = (objeto, string) => {
  let novoArrayJogadores = [];

  let itensEncontrados = 0;

  for (const index in objeto) {
    for (const i in objeto[index]) {
      if (string === i) {
        novoArrayJogadores.push(objeto[index][i]);
...
console.log(funcaoSemana3(arrayJogadores, "nome"));
